...
def feature_subsetter(df: pd.DataFrame,
    ...
    ####################################################################################################################
    metric_rank_str: list[str] = [col for col in df.columns if isinstance(col, str)]
    metric_rank_str: str = [col for col in metric_rank_str if col.startswith('Val_') and col.endswith('Rank')][0]
